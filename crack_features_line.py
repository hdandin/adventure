...
# u_imp = 2.
f_imp = 10e3
width, height, a, r, h1 = 100., 200., 50., 10., 0.8
h2 = 10*h1
meshgenerator = mc.mesh_with_crack
E, nu = 72000., 0.33
hyp2d = "plane stress"
...
### mesh
modelname, physdict = meshgenerator(width*2, height, a, r, h1, h2, translate=(-a,0.), 
                                     modelname='MT_'+str(h1))
# BCs enforce symmetry, no rigid body motion
phys2dirichlet = {'crack_plane':{'y': 0.}, 'crack_tip':{'x': 0.}}
phys2neumann = {'bottom':{'y': -f_imp},'top':{'y': f_imp}}
...
gvdispl = gp.listPlotFieldTri(*pb.postProDisplacement(U), P0=False,
                                viewname='Displacement', VectorType="Displacement",
...
co = Contour(contour_edges, contour_vertices, mesh, xy_g)
fract = fa.FractureAnalysisLine(co, E, nu, [1,2,3,4,5], hyp2d)
a_1_from_j, j = fract.run(U.reshape((-1,2)), stress,
                       ip.UInterpolatorLinear,
                       ip.StressInterpolatorNeighboursMean,
                       "rect", grad_u, strain)
Profile.disable()
