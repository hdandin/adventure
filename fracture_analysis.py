...
class FractureAnalysis:
    ...
    Methods:
        * run - solve for Bueckner-Chen integral
        * getWilliamsStress - reconstruct Williams stress field
        * getWilliamsU - reconstruct Williams displacement field
    """

    def __init__(self, domain: vi.Domain, E: float, nu: float, williams_orders,
    ...
    def run(self, U:np.ndarray, stress:np.ndarray, grad_u:np.ndarray=None, strain:np.ndarray=None):
        """ Solve line integral to compute William's coefficients for given orders """

        integral = vi.EquivalentDomainIntegral(self.domain, self.kappa, self.mu, U, stress,
                                               grad_u, strain)
        ...
        if grad_u is not None:
            j = integral.solve_j()
            k_1 = (j[0]*self.E)**0.5 # contraintes planes
            a_1_from_j = k_1 / (2*np.pi)**0.5
            return a_1_from_j, j[0]

    def williams_coefs_from_integral(self, integral:vi.EquivalentDomainIntegral, n:int, a_aux:float,
                                     b_aux:float):
